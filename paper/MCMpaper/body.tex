...
\left \{
\begin{array}{cl}
\dfrac{(a_{min}a_{max}t_r^2 + 2a_{min}t_rv_{min} + v_{max}^2 - 
2v_{max}v_{min} + v_{min}^2)}{2(a_{min}-a_{max}} & 
\dfrac{v_{min}}{a_{max}}  \geq \dfrac{v_{max}}{a_{min}} + t_r \\
v_{min} t_r + \dfrac{v_{max} ^ 2}{2a_{min}} - \dfrac{
v_{min}^2}{2a_{max}} & \dfrac{v_{min}}{a_{max}}  < \dfrac{v_{max}}
{a_{min}} + t_r
\end{array}
\right .
\end{displaymath}\\
